...
제품 검색·비밀번호 재설정 추가 버전 `c602251498c383e4616675f271ed45b991d97dcc`을 기존 Railway 서비스에 공개 배포했습니다. 130개 테스트와 표준 CI가 성공했고, 실제 I0030 키 조회·가져오기·UL 합산·확인 후 저장·삭제 이력 보존을 확인했습니다. **공개 서비스에서 재설정 메일 수신·코드 확인·새 비밀번호 저장·새 비밀번호 로그인도 사용자가 성공을 확인했습니다.** 인증된 공개 대시보드도 관찰했습니다. [검증 기록](VERIFICATION.md)

**실제 공개 서비스:** [하루영양 열기](https://haru-nutri-production.up.railway.app) · [GitHub 저장소](https://github.com/june-young19/haru-nutri) · [최신 CI](https://github.com/june-young19/haru-nutri/actions/runs/35440578383)

이번 배포에서도 기존 웹·워커·`/app/data` 영구 볼륨·공개 주소를 유지했습니다. 공개 `/api/health`와 `/forgot-password`는 HTTP 200이며, 이전 검증 계정 두 개의 기존 세션·프로필·보호자 분리·활성 제품·삭제 제품 완료 이력 보존 검사도 통과했습니다. 웹에 등록한 `FOOD_SAFETY_API_KEY`로 실제 공식 조회에 성공했습니다. 이름 검색과 설문 후보는 수집 범위가 제한되며, 파서가 읽을 수 없는 함량을 임의로 채우지 않는 점도 실제 결과에서 확인했습니다.

`c602251` 공개 웹의 ACTIVE 상태와 데스크톱 검색·상세·확인 체크·제품명/제조사/세 성분 가져오기·기본 시간 선택기로 01:00 입력·UL 비교 결과·저장 버튼 활성화를 확인했습니다. 사용자 계정의 초안은 저장하지 않았으며 전체 저장·복용·삭제는 별도 계정의 공개 HTTP 검사로 확인했습니다. 신규 검색·재설정 화면의 모바일 검증은 로컬 390×844 범위이며 공개 모바일 검사 성공으로 표시하지 않습니다.

워커도 같은 `c602251`로 ACTIVE를 확인했습니다. 11:52:41 UTC까지 관찰한 주기는 `mode=brevo`, `checked=1`, `sent=0`, `captured=0`, `failed=0`, `skipped=1`로 실패 없이 유지됐습니다. 이는 이번 배포에서 새 복용 알림 메일을 발송·수신했다는 근거가 아닙니다.

이전 단계인 2026-09-19 Brevo 전환 커밋 `a711e5d9cbb04553d7dfff848cc0b407fbdd50db`에서도 표준 CI가 성공했고 Railway 웹 `haru-nutri`와 워커 `haru-reminders`에 배포해 ACTIVE 상태를 확인했습니다. 기존 `/app/data`의 500MB 영구 볼륨과 공개 URL을 유지했습니다. 재배포 후 기존 두 테스트 계정 데이터 보존과 새 두 계정의 가입·제품·성분 합산·복용·삭제 이력·보호자 정보 분리·로그아웃을 공개 HTTPS 검사 3개 그룹으로 확인했습니다. 15:54:41 KST에 웹을 실제 Restart한 뒤에도 세션·프로필·보호자 분리·활성 제품·삭제 제품 완료 기록이 유지되는 것을 확인했습니다.
...
| Railway Trial / Free | 단기 배포 검증에 사용 가능                           | Trial은 최대 30일 또는 $5 소진까지입니다. 이후 Free의 월 $1 크레딧으로 바뀝니다. 두 상시 서비스의 지속 운영을 무료로 보장하지 않습니다. Limited Trial은 외부 네트워크가 제한되어 Brevo API 호출이 실패할 수 있습니다. |
...
### 기존 운영 서비스를 Brevo로 전환

1. 운영 DB를 일관된 방식으로 백업하고 웹·워커가 같은 업데이트 버전을 사용하도록 준비합니다. 기존 볼륨 `/app/data`, `DATABASE_PATH=/app/data/haru.db`, 공개 URL을 유지합니다.
2. Brevo에서 확인한 발신자와 API Key를 준비합니다. 웹 Variables의 메일 설정을 위 네 항목으로 바꾸고 사용하지 않는 이전 공급자 키를 제거합니다. 실제 값은 터미널·로그·문서에 출력하지 않습니다.
3. 워커를 중지한 뒤 웹을 업데이트합니다. DB 버전 3은 알림 재시도 여부 열을 추가하고 기존 데이터를 보존합니다. `/api/health`, 기존 계정 로그인·제품·기록 보존을 확인합니다.
4. 같은 버전의 워커를 재배포합니다. 워커의 `APP_URL`·`CRON_SECRET`은 웹과 일치하게 유지합니다. 웹에만 메일 키를 두고 워커는 집계 로그만 남깁니다.
5. 본인, 독립된 외부 수신자, 동의한 보호자 순서로 새 테스트 일정을 만들어 검사합니다. API 실패나 미확정 전송을 성공으로 바꾸지 않습니다. 각 대상의 실제 수신·폴더·From은 직접 확인한 결과만 기록합니다.
6. 완료·삭제 후 다음 워커 검사에서 발송 대상이 제외되는지 확인합니다. 테스트 제품을 삭제하고 본인 유예 시간을 30분으로 복원합니다.

Brevo에서 IP 오류를 반환하면 Settings → Security → Authorized IPs와 계정 보안 알림을 확인합니다. Railway 발신 IP는 고정이라고 가정하지 않습니다. 자신의 서버 요청임을 확인한 IP만 허용하고 광범위한 보안 해제로 해결하지 않습니다. [Brevo IP 제한](https://help.brevo.com/hc/en-us/articles/5740111683858-Authorize-and-block-IP-addresses-for-API-and-SMTP-security)

## 7. 워커의 재시도와 종료

- 한 프로세스 안에서는 이전 요청이 끝난 후 다음 요청을 예약하여 스캔이 겹치지 않습니다.
- 워커의 HTTP 오류, 네트워크 오류, 잘못된 응답은 다음 주기에 스캔을 재시도합니다. 요청은 최대 55초 후 중단합니다. 이것이 개별 이메일을 무조건 재발송한다는 뜻은 아닙니다.
- `SIGTERM` 또는 `SIGINT`를 받으면 다음 실행 예약을 해제하고 진행 중 HTTP 요청도 취소합니다. 플랫폼은 새 배포나 재시작 정책에 따라 다음 프로세스를 실행합니다.
- 서버는 SQLite의 영구 알림 claim과 동일 알림의 공급자 멱등성 키를 사용합니다. 명백한 API 4xx 요청 거절만 60초 간격으로 총 3회까지 시도합니다. 네트워크·5xx 오류, 중복 키 응답, 성공 응답의 messageId 누락처럼 접수 여부가 불명확하면 실패로 보존하고 자동 재발송을 중단합니다. 15분 이상 미완료인 처리도 같은 방식으로 멈춥니다. 운영자는 공급자 로그에서 먼저 확인해야 합니다.
- 공급자의 멱등성 보관 기간만으로 중복 방지를 보장하지 않습니다. 워커와 서버가 분산 환경의 무조건적인 정확히 한 번 전달을 보장하는 것은 아닙니다.
