...
**회원 탈퇴 배포 완료:** `0d55435`에 본인 비밀번호 확인·최종 확인·개인 서비스 데이터 삭제를 추가했고 **155개 테스트·HTTP 8개 그룹·타입·포맷·빌드·표준 CI**를 통과했습니다. 공개 웹에서 별도 테스트 계정의 실제 탈퇴·기존 로그인 차단·같은 이메일 재가입을 확인했고, 공개 API로 모든 세션 폐기·다른 사용자 보존·새 계정의 빈 상태를 검증했습니다. Railway 웹·워커는 ACTIVE이며 이번 탈퇴 검사의 외부 발송 요청은 0건입니다. [검증 기록](docs/VERIFICATION.md)

**이전 기능 검증:** 공식 건강기능식품 검색·관심 성분 제품 정렬·이메일 비밀번호 재설정 코드는 `c602251`에서 배포·검증했습니다. 당시 표준 CI와 130개 테스트를 통과했고, 실제 식품안전나라 조회·제품 가져오기·기존 등록량 합산·UL 초과 확인·삭제 후 이력 보존을 운영 서비스에서 확인했습니다. **실제 재설정 메일 수신·코드 확인·새 비밀번호 저장·새 비밀번호 로그인도 사용자가 확인했습니다.** 이번 탈퇴 작업에서 외부 이메일 수신을 다시 검사한 것은 아닙니다. [검증 기록](docs/VERIFICATION.md)
...
| `account_deletion_grants` | 사용자·현재 세션·비밀번호 상태에 연결된 5분짜리 탈퇴 권한의 해시    |
...
회원 탈퇴 추가의 버전 5는 `account_deletion_grants`와 요청 제한의 `auth_attempts.owner_hash`를 추가합니다. 마이그레이션 자체가 기존 회원을 탈퇴시키거나 DB를 초기화하지 않습니다. 실제 데이터 삭제는 인증된 사용자가 탈퇴의 최종 확인을 완료했을 때만 수행합니다.

**롤백 시 주의:** v4까지만 지원하는 구버전 서버는 v5 DB를 발견하면 버전 검사로 실행을 거절합니다. 기능 커밋을 revert하는 것만으로 v5 DB를 구버전에서 사용할 수는 없습니다. DB 버전을 강제로 낮추지 말고 v5를 읽을 수 있는 코드로 수정·배포하세요. 예전 DB 백업 복원은 이후 탈퇴한 계정과 알림을 되살릴 수 있으므로 서비스·워커를 다시 열기 전에 삭제 상태를 별도로 확인해야 합니다.
...
설정 아래 **위험 영역 → 회원 탈퇴 → `/settings/delete-account` 삭제 범위 확인 → 현재 비밀번호 확인 → 최종 확인 체크와 탈퇴 버튼** 순서로 진행합니다. 비밀번호를 잊었다면 먼저 기존 이메일 비밀번호 재설정을 이용합니다. 최종 확인 전에는 취소할 수 있고, 처리 중 중복 클릭은 막습니다. 서버가 현재 로그인 세션의 사용자만 대상으로 비밀번호를 다시 확인합니다.

성공하면 계정·모든 로그인 세션·활성/보관 영양제·성분·복용 일정·완료 기록·설문 응답·보호자 이메일/동의·알림 설정/내역·비밀번호 재설정 요청과 임시 권한을 하나의 DB 트랜잭션으로 삭제합니다. 일반 영양제 삭제의 과거 기록 보관 정책은 회원 탈퇴에 적용하지 않습니다. 같은 이메일로 다시 가입할 수 있지만 새 사용자 ID를 받으며 이전 기록은 복원되지 않습니다.

운영 DB의 행 삭제와 저장장치·백업·메일 공급자 기록의 물리 소거는 다릅니다. 이 기능에 백업 원본이나 이미 전달된 메일의 회수·삭제 기능은 없습니다. 탈퇴가 완료된 사용자는 이후 알림 조회에서 제외하며 이미 시작된 외부 발송은 취소를 보장하지 않습니다. 구버전의 일부 불투명 요청 제한 해시는 만료까지 남을 수 있습니다. [정확한 삭제 범위와 한계](docs/SECURITY.md#회원-탈퇴와-보관-범위)

이번 기능의 자동 테스트·실제 UI·공개 배포 완료 여부는 [검증 기록](docs/VERIFICATION.md)을 확인하세요. 개인 운영 계정을 시험 삼아 삭제하지 않습니다.
