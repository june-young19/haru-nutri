...
[GitHub 저장소](https://github.com/june-young19/haru-nutri) · [외부 배포 가이드](docs/DEPLOYMENT.md) · [시연 가이드](docs/DEMO.md)

현재 실제 공개 서비스 URL은 검증 전입니다. GitHub 저장소 생성과 소스 배포 준비를 공개 웹서비스의 운영 완료로 표시하지 않습니다.
...
이 실행 환경에서는 표준 `pnpm test`의 자식 프로세스 생성이 `spawn EPERM`으로 제한되어, 동일한 테스트 소스를 컴파일한 후 Node의 프로세스 격리를 끄고 총 62개를 실행했습니다. 표준 명령 자체가 이 환경에서 성공했다고 표시하지 않습니다. 워커 검사는 종료·재시도·비중첩·설정 검증·비밀값 로그 방지를 포함합니다.

이전 실제 Resend 이메일 1통의 공급자 대시보드 `Delivered`를 확인했습니다. 사용자의 받은편지함 확인은 아직 완료하지 않았습니다. 이후 키를 교체했고 새 키를 이용한 외부 발송은 아직 수행하지 않았습니다. GitHub 원격 저장소 생성, Railway 웹 서비스·`/app/data` 볼륨·환경변수 준비를 마쳤으며 전체 소스 업로드와 외부 호스팅 배포는 진행 중입니다. Docker 컨테이너 실행과 공개 서비스 URL 검증은 아직 완료하지 않았습니다. 소스에 개인 API Key나 수신자 주소를 포함하지 않습니다.
