...
**이번 추가 작업:** 공식 건강기능식품 검색·관심 성분 제품 정렬·이메일 비밀번호 재설정을 기존 프로젝트에 연결합니다. 작업 전 기준 커밋은 `b1d6bfd`입니다. 아래 기존 배포·복용 알림 검증과 달리, 새 공식 API Key를 이용한 실제 검색과 비밀번호 재설정 이메일·운영 변경 검증은 아직 완료되지 않았습니다. [추가 기능 검증 상태](docs/VERIFICATION.md)
...
이번 제품 검색·비밀번호 재설정 추가 작업은 체크포인트 `b1d6bfd`에서 시작했습니다. 새 정렬 테스트 14개는 strict TypeScript 컴파일과 실제 실행을 통과했습니다. 전체 회귀 테스트·실제 공식 키 검색·새 재설정 메일 수신·운영 비밀번호 변경 검증은 진행 중이며, 아래 73개와 복용 알림 실측은 이번 추가 전의 검증 기록입니다. 최신 상태는 [검증 기록](docs/VERIFICATION.md)을 따릅니다.
